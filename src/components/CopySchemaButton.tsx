...
// SQL Schema - Complete content from migration file
const SCHEMA_SQL = `-- ============================================
...
export function CopySchemaButton({ onCopy }: CopySchemaButtonProps) {
  ...
  const fallbackCopy = () => {
    ...
    try {
      ...
      textarea.select();
      textarea.setSelectionRange(0, SCHEMA_SQL.length);
  ...
  const handleDownload = () => {
    ...
    const blob = new Blob([SCHEMA_SQL], { type: 'text/plain' });
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <Button
          ...
          className={copied ? "bg-green-600 hover:bg-green-700 flex-1" : "flex-1"}
          size="lg"
        >
          {copied ? (
            <>
      ...
      {error && (
        <div className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3 text-sm">
          <p className="text-yellow-800 dark:text-yellow-200">
            <strong>⚠️ Copy failed!</strong> Try the download button instead, or manually copy from the text box below.
          </p>
        </div>
      )}
    </div>
