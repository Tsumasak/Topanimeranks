...
export function AnimeHero({ anime }: AnimeHeroProps) {
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: 'var(--background)' }}>
      ...
      {/* Content */}
      <div className="relative z-10 container mx-auto px-[24px] py-12">
        {/* Breadcrumb */}
        <AnimeBreadcrumb 
          season={anime.season} 
          year={anime.year} 
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start mt-6">
          ...
          <div className="flex-1 space-y-4">
            {/* Title */}
            <div>
              <h1 className="text-5xl mb-2 font-bold" style={{ color: 'var(--foreground)' }}>
                {anime.title_english || anime.title}
              </h1>
              {anime.title_japanese && (
                <p className="text-xl" style={{ color: 'var(--rating-text)' }}>
                  {anime.title_japanese}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4" style={{ color: 'var(--foreground)' }}>
              {(anime.score || anime.anime_score) && (
                <div className="flex items-center gap-2">
                  <span className="text-lg">⭐</span>
                  <span className="text-lg">{(anime.score || anime.anime_score)?.toFixed(1)}</span>
                </div>
              )}
              {anime.members && (
                <div className="flex items-center gap-2">
                  <span className="text-lg">👥</span>
                  <span className="text-lg">{formatNumber(anime.members)}</span>
                </div>
              )}
              {anime.favorites && (
                <div className="flex items-center gap-2">
                  <span className="text-lg">💛</span>
                  <span className="text-lg">{formatNumber(anime.favorites)}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {anime.type && (
                <span className={`${getTypeClass(anime.type)} px-3 py-1 rounded-full text-xs`}>
                  {anime.type}
                </span>
              )}
              ...
              {anime.status && (
                <span 
                  className={`px-3 py-1 rounded-full text-xs ${
                    anime.status === 'Currently Airing' ? 'tag-ona' : 'tag-default'
                  }`}
                >
                  {anime.status}
                </span>
              ...
              {anime.demographics && anime.demographics.length > 0 && anime.demographics.map((demo: any, index: number) => {
